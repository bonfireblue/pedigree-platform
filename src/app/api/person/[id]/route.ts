...
export async function GET(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    console.error("GET /api/person/[id] failed", error);
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    console.error("PATCH /api/person/[id] failed", error);
    return NextResponse.json({ error: "INTERNAL_SERVER_ERROR" }, { status: 500 });
  }
}

// DELETE endpoint temporarily removed - will be added back via separate route
